...
namespace twl::gfx {
    ...
    namespace {
        ...
        void JoinBlocksImpl(u32 &cur_width, u32 &cur_height, const u32 def_width, abgr8888::Color *&out_rgba) {
            ...
            auto new_rgba = new abgr8888::Color[new_width * new_height]();
        ...
        Result ConvertGraphicsToRgbaImpl(GraphicsToRgbaContext &ctx, const bool do_join_blocks) {
            ...
            ctx.out_rgba = new abgr8888::Color[cur_width * cur_height]();
            ...
            switch(ctx.pix_fmt) {
                ...
                case PixelFormat::Palette4: {
                    ...
                    if(!CheckBpp(bpp, ctx.gfx_data_size, ctx.def_width, ctx.def_height)) {
                        TWL_R_FAIL(ResultInvalidSizeForPixelFormat);
                    }
                    switch(ctx.char_fmt) {
                ...
                default: {
                    ...
                    TWL_R_FAIL(ResultUnsupportedPixelFormat);
                }
            }
        ...
        Result ConvertRgbaToGraphicsImpl(RgbaToGraphicsContext &ctx) {
            ...
            switch(ctx.pix_fmt) {
                case PixelFormat::Palette4: {
                    constexpr u32 bpp = 2;
                    auto out_plt = new xbgr1555::Color[plt_count]();
                    ...
                    ctx.out_gfx_data = new u8[gfx_size]();
                                    ...
                                    for(size_t y = 0; y < TileSize; y++) {
                                        for(size_t x = 0; x < TileSize; x += 4) {
                                            ...
                                            if(!HandleColorInPalette(out_plt, plt_count, out_plt_allocated, clr_1, plt_idx_1)) {
                                                TWL_R_FAIL(ResultColorPaletteAllocationFailure);
                                            }
                                            u8 plt_idx_2;
        ...
        Result ConvertGraphicsToRgbaWithScreenDataImpl(GraphicsToRgbaContext &ctx) {
            if((ctx.pix_fmt != PixelFormat::Palette16) && (ctx.pix_fmt != PixelFormat::Palette256)) {
                TWL_R_FAIL(ResultInvalidPixelFormat);
            }
            if(ctx.char_fmt != CharacterFormat::Char) {
                TWL_R_FAIL(ResultInvalidCharacterFormat);
            }
            ...
            ctx.out_rgba = new abgr8888::Color[tmp_width * tmp_height]();
            ...
            auto tmp_conv_array = new GraphicsToRgbaContext[plt_count]();
                ...
                if(cur_val.tile_number != 0) {
                    ...
                    for(size_t y = 0; y < TileSize; y++) {
                        for(size_t x = 0; x < TileSize; x++) {
                            ...
                            if(tmp_rgba == nullptr) {
                                TWL_R_FAIL(ResultInvalidScreenData);
                            }
    ...
    Result ConvertRgbaToGraphics(RgbaToGraphicsContext &ctx) {
        if(ctx.gen_scr_data) {
            // TODO: support this
            TWL_R_FAIL(ResultScreenDataGenerationUnimplemented);
        }
        else {
